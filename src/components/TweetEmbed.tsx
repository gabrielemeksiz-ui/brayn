  ...
  return (
    <div className="space-y-3">
      ...
      <a
        ...
        className="flex items-center gap-3 rounded-[6px] border border-[#2A2A2A] bg-[#1E1E1E] px-3 py-2 hover:border-[#333] hover:bg-[#222] transition-colors duration-100 no-underline group"
      >
        <div className="shrink-0 w-6 h-6 rounded-full bg-black flex items-center justify-center border border-[#2A2A2A]">
        ...
        <span className="text-[13px] font-medium text-[#D4D4D4] group-hover:text-white transition-colors truncate">{data.authorName}</span>
        <span className="text-[12px] text-[#606060] shrink-0">@{data.authorHandle}</span>
        <svg className="ml-auto shrink-0 text-[#444]" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/>
        </svg>
      </a>

      {/* Full tweet text */}
      <p className="text-[14px] text-[#C8C8C8] leading-relaxed whitespace-pre-wrap px-1">{data.text}</p>
    </div>
  );
}
